...
export type LevelSeedDocument = {
  ...
  groundPlane: LevelDefinition['groundPlane'];
  groups: [];
};
...
export const buildLevelSeedDocument = (build: LevelBuild): LevelSeedDocument => {
  ...
  return {
    ...
    groundPlane: {
      position: [...(definition.groundPlane?.position ?? DEFAULT_LEVEL_GROUND_PLANE.position)] as [
        number,
        number,
        number,
      ],
      size: definition.groundPlane?.size ?? DEFAULT_LEVEL_GROUND_PLANE.size,
      variant: definition.groundPlane?.variant ?? DEFAULT_LEVEL_GROUND_PLANE.variant,
    },
    groups: [],
  };
};
