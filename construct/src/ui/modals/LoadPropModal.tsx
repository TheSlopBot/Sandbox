...
export type LoadPropModalProps = {
  entries: PropLocalStoreEntry[];
  onCancel: () => void;
  onSelect: (entry: PropLocalStoreEntry) => void;
  onDelete: (entry: PropLocalStoreEntry) => void;
};

export const LoadPropModal = ({
  entries,
  onCancel,
  onSelect,
  onDelete,
}: LoadPropModalProps) => {
  const [pendingDelete, setPendingDelete] = useState<PropLocalStoreEntry | null>(null);
    ...
    <>
      <div className="construct-modalBackdrop" role="presentation" onMouseDown={onCancel}>
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="construct-load-title"
          onMouseDown={(e) => e.stopPropagation()}
        >
          <div className="construct-modalTitle" id="construct-load-title">
            Load prop
          </div>
          {entries.length === 0 ? (
            <div className="mutedNote">No props saved in local storage.</div>
          ) : (
              ...
              {entries.map((entry) => (
                <div key={entry.id} className="construct-modalListRow">
                  ...
                  >
                    ...
                    <span className="construct-subtle">{entry.id}.prop</span>
                  </button>
                  <button
                    type="button"
